...
<html>
<head>
    <title>PT Contributions Dashboard</title>
...
<<<<<<< HEAD
            <div class="d-flex justify-content-between align-items-center p-3 header-bar">
                <h2 class="mb-0"><i class="fas fa-file-invoice-dollar me-3"></i>PT Contributions Dashboard</h2>
                <button class="btn" data-bs-toggle="modal" data-bs-target="#ptModal">
=======
            <div class="d-flex justify-content-between align-items-center p-3 rounded-3 header-bar card-3d">
                <h2 class="mb-0"><i class="fas fa-file-invoice-dollar me-3"></i>PT Contributions Dashboard</h2>
                <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#ptModal">
>>>>>>> 0be22774be56f74f236fbb15502732a798d923a4
                    <i class="fas fa-plus me-2"></i>Add PT Contribution
                </button>
            </div>
        </div>
    </div>
...
>>>>>>> 0be22774be56f74f236fbb15502732a798d923a4
                <table id="ptTable" class="table table-hover align-middle" style="width:100%;">
                    <thead>
                        <tr>
                            <th><i class="fas fa-briefcase me-2"></i>Profit Center</th>
                            <th><i class="fas fa-calendar-alt me-2"></i>Period</th>
                            <th class="text-end"><i class="fas fa-file-invoice-dollar me-2"></i>PT</th>
...
>>>>>>> 0be22774be56f74f236fbb15502732a798d923a4
                    ...
                    <tbody>
                        <c:forEach items="${ptList}" var="pt" varStatus="index">
                            <tr class="table-row">
                                <td data-label="Profit Center">
                                    <div class="fw-bold">${pt.profit_center_code}</div>
<<<<<<< HEAD
                                    <div class="small" style="color: var(--text-muted-color) !important;">${pt.profit_center_name}</div>
=======
                                    <div class="small" style="color: var(--muted-color) !important;">${pt.profit_center_name}</div>
>>>>>>> 0be22774be56f74f236fbb15502732a798d923a4
                                </td>
                                <td data-label="Period" class="fw-bold">${pt.month_year}</td>
                                <td data-label="PT" class="text-end">₹${pt.pt}</td>
                                <td data-label="Total" class="text-end fw-bold">₹${pt.total_amount}</td>
                                <td data-label="Paid" class="text-end text-success">₹${pt.amount_paid}</td>
                                <td data-label="Balance" class="text-end text-danger">₹${pt.difference}</td>
                                <td data-label="Due Date">${pt.due_date}</td>
                                <td data-label="Paid Date">${not empty pt.actual_payment_date ? pt.actual_payment_date : '-'}</td>
                                <td data-label="Days Diff">
                                    <c:choose>
                                        <c:when test="${not empty pt.actual_payment_date && pt.delay_days > 0}">
                                            <span class="badge bg-danger">${pt.delay_days} days</span>
                                        </c:when>
                                        <c:when test="${not empty pt.actual_payment_date}">
                                            <span class="badge bg-success">${pt.delay_days} days</span>
                                        </c:when>
                                        <c:otherwise>-</c:otherwise>
                                    </c:choose>
                                </td>
                                ...
                                <c:choose>
                                    <c:when test="${sessionScope.ROLE eq 'Admin' or sessionScope.ROLE eq 'SA'}">
                                        <td data-label="Status">
                                            <span class="status-badge ${pt.status == 'Active' ? 'active' : 'inactive'}">
                                                <span class="status-text">${pt.status}</span>
                                            </span>
                                        </td>
                                        <td class="text-center" data-label="Actions">
                                            <div class="d-flex justify-content-center">
                                                <button class="btn btn-sm btn-outline-primary me-1 btn-action" data-bs-toggle="modal" data-bs-target="#ptModal_${index.count}">
...
=======
                                 <c:choose>
                                  <c:when test="${sessionScope.ROLE eq 'Admin' or sessionScope.ROLE eq 'SA'}">
                                      ...
                                      <td class="text-center" data-label="Actions">
                                        <div class="d-flex justify-content-center">
                                            <button class="btn btn-sm btn-outline-primary me-1 btn-action" data-bs-toggle="modal" data-bs-target="#ptModal${index.count }">
                                                 <i class="fas fa-edit"></i>
                                            </button>
                                        </div>
                                      </td>
            ...
            <div class="modal-content">
                <form id="ptForm_${index.count}" action="<%=request.getContextPath() %>/update-pt" method="post">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="row mb-4 g-3">
                            <c:choose>
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <select class="form-select select2" name="profit_center_code" id="profitCenterSelect_${index.count}" required>
                                            ...
                                            <c:forEach items="${profitCenterList}" var="pc">
                                                <option value="${pc.profit_center_code}" ${pc.profit_center_code == pt.profit_center_code ? 'selected' : ''}>${pc.profit_center_code} - ${pc.profit_center_name}</option>
                                            </c:forEach>
                                        </select>
                                    </div>
                                </c:when>
                                ...
                                <c:otherwise>
                                    <div class="col-md-6">
                                        <label class="form-label fw-bold">Profit Center</label>
                                        <p class="form-control-plaintext bg-light p-2 rounded">${pt.profit_center_code} - ${pt.profit_center_name}</p>
                                        <input type="hidden" name="profit_center_code" value="${pt.profit_center_code}" />
                                    </div>
                                </c:otherwise>
                            </c:choose>
                            <div class="col-md-6">
                                <label class="form-label fw-bold">Month-Year</label>
                                <p class="form-control-plaintext bg-light p-2 rounded">${pt.month_year}</p>
                                <input type="hidden" name="month_year" value="${pt.month_year}" />
                            </div>
                        </div>
                        ...
                        <div class="row mb-3 g-3">
                            <div class="col-md-4">
                                ...
                                <input type="number" step="0.01" min="0" id="pt_contribution_${index.count}" name="pt" class="form-control" value="${pt.pt}" oninput="calculateTotal_edit(${index.count})" required />
                            </div>
                            <div class="col-md-4">
                        ...
                        <div class="row mb-3 g-3">
                            ...
                            <div class="col-md-3">
                                ...
                                <select name="status" class="form-select">
                                    ...
                                    <option value="Inactive" ${pt.status == 'Inactive' ? 'selected' : ''}>Inactive</option>
                                </select>
                            </div>
                        </div>
                    </div>
                    <div class="modal-footer">
                        ...
                        <button type="submit" class="btn btn-primary">Update PT Contribution</button>
                    </div>
                </form>
            </div>
        </div>
...
<!-- MODALS -->
<c:forEach items="${ptList}" var="pt" varStatus="index">
  <div class="modal fade" id="ptModal${index.count}" tabindex="-1" aria-labelledby="ptModalLabel${index.count}" aria-hidden="true">
    <div class="modal-dialog modal-xl modal-dialog-centered">
      <div class="modal-content">
            <form id="ptForm${index.count }" action="<%=request.getContextPath() %>/update-pt" method="post">
                <div class="modal-header header-bar" style="border-top-left-radius: 15px; border-top-right-radius: 15px;">
                    <h5 class="modal-title" id="ptModalLabel">Update PT Contribution</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                  <input type="hidden" id="pt_id" name="id" value="${pt.id}" />
					<div class="row mb-3">
					    <div class="col-md-6">
					        <div class="form-group">
					            <label for="profitCenterSelect${index.count}">Profit Center</label>
					            <select id="profitCenterSelect${index.count}" name="profit_centers" class="form-control select2">
					                <option value="">- Search Profit Center -</option>
					                <c:forEach items="${profitCenterList}" var="pc">
					                    <option value="${pc.profit_center_code}" ${pt.profit_center_code == pc.profit_center_code ? 'selected' : ''}>
					    ...
					    <div class="col-md-6">
					        <div class="form-group">
					            <label for="monthYear${index.count}">Month-Year</label>
					            <input type="month" id="monthYear${index.count}" name="month_years" class="form-control" value="${pt.month_year}" />
					        </div>
					    </div>
					</div>
					<div class="row mb-3 bg-light p-3 rounded-3 align-items-end">
					    <div class="col-md-3">
					        <div class="form-group">
					            <label>PT (₹)</label>
					            <div class="input-group input-group-sm">
					                <span class="input-group-text">₹</span>
					                <input type="number" step="0.01" id="pt_contribution${index.count}" name="pt" 
					                       class="form-control form-control-sm" value="${pt.pt}" 
					    ...
					    <div class="col-md-2">
					        <div class="form-group">
					            ...
					            <div class="input-group input-group-sm">
					                <span class="input-group-text">₹</span>
					                <input type="number" id="total_amount${index.count}" name="total_amount"  value="${pt.total_amount}"
					    ...
					    <div class="col-md-3">
					        <div class="form-group">
					            ...
					            <div class="input-group input-group-sm">
					                <span class="input-group-text">₹</span>
					                <input type="number" step="0.01" id="amount_paid${index.count}" name="amount_paid" 
					                       class="form-control form-control-sm" value="${pt.amount_paid}" 
					    ...
					    <div class="col-md-2">
					        <div class="form-group">
					            ...
					            <div class="input-group input-group-sm">
					                <span class="input-group-text">₹</span>
					                <input type="number" id="difference${index.count}" name="difference" value="${pt.difference}" 
					                       class="form-control form-control-sm bg-white" readonly required />
					            </div>
					        </div>
					    </div>
					...
					<div class="row mb-3">
					    <div class="col-md-4">
					        <div class="form-group">
					            ...
					            <input type="date" id="due_date${index.count}" name="due_date" class="form-control" 
					                   value="${pt.due_date}" required />
					        </div>
					    </div>
					    <div class="col-md-4">
					        <div class="form-group">
					            <label for="actual_payment_date${index.count}" class="form-label fw-bold">Actual Payment Date</label>
					            <input type="date" id="actual_payment_date${index.count}" name="actual_payment_date" 
					                   class="form-control" value="${pt.actual_payment_date}" 
					                   onchange="calculateDelayDays1(${index.count})" />
					        </div>
					    </div>
					    <div class="col-md-4">
					        <div class="form-group">
					            <label for="delay_days${index.count}" class="form-label fw-bold">Delay in Days</label>
					            <input type="number" id="delay_days${index.count}" name="delay_days" 
					                   class="form-control bg-light" value="${pt.delay_days}" readonly />
					        </div>
					    </div>
					</div>
					<div class="row mb-3">
					    <div class="col-md-4">
					        <div class="form-group">
					            <label for="challan_no${index.count}">Challan Number</label>
					            <input type="text" id="challan_no${index.count}" name="challan_no" class="form-control" value="${pt.challan_no}" placeholder="Enter challan number" />
					        </div>
					    </div>
					    <div class="col-md-4">
					        <div class="form-group">
					            <label for="no_of_emp${index.count}">Count of Employees</label>
					            <input type="text" id="no_of_emp${index.count}" name="no_of_emp" class="form-control" value="${pt.no_of_emp}" placeholder="Enter no of emp count. ie., 17" />
					        </div>
					    </div>
					    <div class="col-md-4">
					        <div class="form-group">
					            <label for="status${index.count}">Status</label>
                ...
                <div class="modal-footer" style="border-bottom-left-radius: 15px; border-bottom-right-radius: 15px;">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                    <button type="submit" class="btn btn-primary" id="submitBtn${index.count}" >Update PT Contribution</button>
                </div>
            </form>
        </div>
    </div>
...
<<<<<<< HEAD
                    ...
                    <div class="row mb-3 g-3">
                        <div class="col-md-4">
                            <label class="form-label">PT (₹)</label>
                            <input type="number" step="0.01" min="0" class="form-control" id="pt_contribution" name="pt" oninput="calculateTotal()" required>
                        </div>
                        <div class="col-md-4">
                            <label class="form-label fw-bold">Total (₹)</label>
                            <input type="number" class="form-control bg-light" id="total_amount" name="total_amount" readonly required>
                        </div>
                    </div>
                    ...
                    <div class="row mb-3 g-3">
                        <div class="col-md-4">
                            <label class="form-label">Amount Paid (₹)</label>
                            <input type="number" step="0.01" min="0" class="form-control" id="amount_paid" name="amount_paid" oninput="calculateDifference()" required>
                        </div>
                        <div class="col-md-4">
                            <label class="form-label fw-bold">Balance (₹)</label>
                            <input type="number" class="form-control bg-light" id="difference" name="difference" readonly required>
                        </div>
                        <div class="col-md-4">
                            <label class="form-label">Due Date</label>
                        ...
                        <div class="col-md-4">
                            <label class="form-label">Actual Payment Date</label>
                            <input type="date" class="form-control actual-payment-date-input" id="actual_payment_date" name="actual_payment_date" onchange="calculateDelayDays()">
                        </div>
                        <div class="col-md-4">
                            <label class="form-label">Delay in Days</label>
                            <input type="number" class="form-control bg-light" id="delay_days" name="delay_days" readonly value="0">
                        </div>
                        <div class="col-md-4">
                            <label class="form-label">Challan Number</label>
...
=======
                    ...
                    <div class="row mb-3 bg-light p-3 rounded-3 align-items-end">
                         <div class="col-md-3"><label class="form-label">PT (₹)</label><div class="input-group input-group-sm"><span class="input-group-text">₹</span><input type="number" step="0.01" class="form-control form-control-sm" id="pt_contribution" name="pt" oninput="calculateTotal()" required></div></div>
                         ...
                         <div class="col-md-3"><label class="form-label">Amount Paid (₹)</label><div class="input-group input-group-sm"><span class="input-group-text">₹</span><input type="number" step="0.01" class="form-control form-control-sm" id="amount_paid" name="amount_paid" oninput="calculateDifference()" required></div></div>
                    ...
                    <div class="row mb-3">
                         ...
                         <div class="col-md-4"><label class="form-label fw-bold">Actual Payment Date</label><input type="date" class="form-control" name="actual_payment_date" id="actual_payment_date" onchange="calculateDelayDays()"></div>
                         <div class="col-md-4"><label class="form-label fw-bold">Delay in Days</label><input type="number" class="form-control bg-light" name="delay_days" id="delay_days" readonly value="0"></div>
                    </div>
                    <div class="row mb-3">
                        <div class="col-md-6"><label class="form-label fw-bold">Challan Number</label><input type="text" class="form-control" name="challan_no" id="challan_no" placeholder="Enter challan number"></div>
                        <div class="col-md-6"><label class="form-label fw-bold">Count of Employees this month.</label><input type="text" class="form-control" name="no_of_emp" id="no_of_emp" placeholder="Enter no of emp count. ie,. 17"></div>
                    </div>
                </div>
                <div class="modal-footer" style="border-bottom-left-radius: 15px; border-bottom-right-radius: 15px;">
>>>>>>> 0be22774be56f74f236fbb15502732a798d923a4
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                    <button type="submit" class="btn btn-primary" id="submitBtn">Save PT Contribution</button>
                </div>
            </form>
        </div>
    </div>
...
=======
        ...
        // Correctly initialize Select2 for all modals
>>>>>>> 0be22774be56f74f236fbb15502732a798d923a4
        $('#ptModal .select2').select2({
            theme: 'bootstrap-5',
            dropdownParent: $('#ptModal')
        });
<<<<<<< HEAD
        $('[id^=ptModal_] .select2').each(function() {
            $(this).select2({
                theme: 'bootstrap-5',
                dropdownParent: $(this).closest('.modal')
=======
        
        // Loop through each update modal and initialize its Select2
        $('[id^=ptModal]').each(function() {
...
<<<<<<< HEAD
    ...
    $('#ptModal').on('show.bs.modal', function (e) {
        if (!$(e.relatedTarget).hasClass('edit-trigger')) {
            $('#ptForm')[0].reset();
            $('#ptModalLabel').text('Add New PT Contribution');
...
function calculateTotal1(index) {
    var pt = parseNumericInput('#pt_contribution' + index);
    $('#total_amount' + index).val(pt.toFixed(2));
...
function calculateTotal() { var total = parseNumericInput('#pt_contribution'); $('#total_amount').val(total.toFixed(2)); calculateDifference(); }
...
function confirmDelete(id) { Swal.fire({ title: 'Are you sure?', text: "You won't be able to revert this!", icon: 'warning', showCancelButton: true, confirmButtonColor: '#d33', cancelButtonColor: '#3085d6', confirmButtonText: 'Yes, delete it!' }).then(function(result) { if (result.isConfirmed) { Swal.fire('Deleted!','The PT contribution has been deleted.','success'); } }); }
...
function checkDuplicatePCMY() {
    ...
    if (pcVal !== '' && myVal !== '') {
        ...
        $('#ptTable').DataTable().rows().every(function() {
