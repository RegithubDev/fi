...
public class ContributionDao {
		...
		DataSourceTransactionManager transactionManager;
	 	public List<PfContribution> getPfContributions(String monthYear, String entity, String profitCenter, User usr) {
	        String sql = "SELECT pf.[id]\r\n"
	        		...
	        		+ "      ,pf.[delay_days]\r\n"
	        		+ "      ,pf.[challan_no],no_of_emp,pf.status,pf.upload_file, pf.remarks FROM pf_contributions pf "
	        ...
	        return jdbcTemplate.query(sql, params.toArray(), new BeanPropertyRowMapper<>(PfContribution.class));
	    }
	 	 ...
	 	 public boolean addPfContribution(PfContribution pf) {
			...
			TransactionStatus status = transactionManager.getTransaction(def);
	 		 try {
	 			...
	 			String insertQry = "INSERT INTO [pf_contributions] "
						+ "(created_by,profit_center_code, month_year, employee_contribution, vpf,employer_contribution, pf_admin,total_amount,amount_paid,difference,challan_no, "
						+ "pf_edli,due_date, actual_payment_date,no_of_emp,status,created_date,upload_file,remarks)"
						+ " VALUES "
						+ "(:created_by, :profit_center_code, :month_year, :employee_contribution, :vpf,:employer_contribution, :pf_admin, :total_amount, :amount_paid, :difference,:challan_no, "
						+ ":pf_edli,:due_date, :actual_payment_date, :no_of_emp, :status, getdate(), :upload_file, :remarks)";
	     ...
	     public boolean updatePfContribution(PfContribution pf) {
	    	 int count = 0;
				...
				TransactionStatus status = transactionManager.getTransaction(def);
		 		 try {
		 			NamedParameterJdbcTemplate namedParamJdbcTemplate = new NamedParameterJdbcTemplate(dataSource);
		 			String uQry = "UPDATE [pf_contributions] SET "
		 				    ...
		 				    + ", status = :status,remarks =:remarks, modified_date = getdate(),modified_by =:modified_by, upload_file= :upload_file "
		 				    + " WHERE id = :id";
		...
		public List<PfContribution> geteList() throws Exception {
			...
			try {
				...
				objsList = jdbcTemplate.query( qry, new BeanPropertyRowMapper<PfContribution>(PfContribution.class));
			} catch (Exception e) {
				e.printStackTrace();
